/*
Author : Aman Singh
Solution for problem code meet_middle.
...
ll rec(vi &arr, vector<ll> &res, ll curr, ll sum, int last){
	if(curr == last) res.pb(sum);
	else{
		rec(arr, res, curr + 1, sum, last);
		rec(arr, res, curr + 1, sum + arr[curr], last);
	}
}
...
int solve(){
	...
	if(n <= 20){
		rec(arr, res, 0, 0, n);
		sort(all(res));
		ll x = lower_bound(all(res), target) - res.begin();
		if(target == res[x]) return target;
		else return res[x - 1];
	}
